...
@Slf4j
public class CreateTopUpConsumer {
    ...
    @KafkaListener(topics = "create-transfer-topic", groupId = "orchestrator-service-group")
    public void consume(Event event) {
            service.handleTopUpCreate(event);
    }


    @DltHandler
    public void handleDlt(Event id) {
        log.error("Create transfer topic error {}",id);
    }
